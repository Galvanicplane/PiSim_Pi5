#!/usr/bin/env python3
"""
video_receiver.py - Live FPV Camera Stream Receiver for Raspberry Pi 5
Receives JPEG compressed camera frames over UDP Port 5000 from UE5 actor and displays using OpenCV.
"""
...
class VideoReceiver:
    ...
    Receives compressed JPEG frames over UDP (Port 5000) from PiSim (Unreal Engine 5)
    and displays live video stream using OpenCV.
    """
    def __init__(self, listen_ip: str = "0.0.0.0", listen_port: int = 5000):
        ...
        print(f"[VideoReceiver] Listening for live FPV video stream on {self.listen_ip}:{self.listen_port}...")
    ...
    def start_display_loop(self, frame_callback=None):
        ...
        """
        window_name = "PiSim Pi5 Live FPV Camera Stream (Port 5000)"
            ...
            while True:
                ...
                if frame is not None:
                    ...
                    if frame_count % 30 == 0:
                        ...
                        print(f"[VideoReceiver] FPV Streaming active... FPS: {fps:.1f} ({frame.shape[1]}x{frame.shape[0]})")
...
if __name__ == "__main__":
    def my_image_processing_example(frame):
        # Optional timestamp / overlay text
        cv2.putText(frame, "Pi5 FPV Live Stream", (10, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        return frame

    receiver = VideoReceiver(listen_ip="0.0.0.0", listen_port=5000)
